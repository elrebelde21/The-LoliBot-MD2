let handler = m => m
handler.all = async function (m) {
let chat = global.db.data.chats[m.chat]

if (/^sus$/i.test(m.text) && chat.audios) {
let vn = './media/sus.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(el amor|😍)/gi)) {    
let vn = './media/el amor.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(estupido|estúpido)/gi)) {    
let vn = './media/estupido.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(fiu fiu|mi bebito fiu fui|bebito fui fui|fui fui)/gi)) {    
let vn = './media/fiu.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(No soy pati|no soy pati)/gi)) {    
let vn = './media/no soy pati.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(oxxo|oxxo mmm)/gi)) {    
let vn = './media/oxxo.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Potaxio|potacio)/gi)) {    
let vn = './media/Potaxio.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Que linda noche|que linda noche)/gi)) {    
let vn = './media/que linda noche.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(sabosito|sabosito)/gi)) {    
let vn = './media/sabosito.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Te elimino|te elimino)/gi)) {    
let vn = './media/te elimino..mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Te pasas|te pasas)/gi)) {    
let vn = './media/te pasas.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Te sabes|te sabes)/gi)) {    
let vn = './media/te sabes.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Temon|temon)/gi)) {    
let vn = './media/temon.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Trabajo|mañana no trabajo)/gi)) {    
let vn = './media/trabajo.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(mami yo que digo a ti|que digo)/gi)) {    
let vn = './media/mami yo que digo a ti.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Miku|miku)/gi)) {    
let vn = './media/miku.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Mujer|dama|mujeres)/gi)) {    
let vn = './media/mujer.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(paja|pajin)/gi)) {    
let vn = './media/pajin.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Te felicito|te felicito que bien altual)/gi)) {    
let vn = './media/te felicito.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}

if (chat.audios && m.text.match(/(Yokese|YOKESE|yokesee)/gi)) {    
let vn = './media/Yokese.mp3'
this.sendPresenceUpdate('recording', m.chat)   
this.sendMessage(m.chat, { audio: { url: vn }, fileName: 'error.mp3', mimetype: 'audio/mp4', ptt: true }, { quoted: m })}
   
return !0 }
export default handler
